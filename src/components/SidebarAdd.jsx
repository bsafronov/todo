...
function SidebarAdd({ db, data, setData, setActiveIndex }) {
  ...
  function addFolder() {
    ...
    const name = folderName[0].toUpperCase() + folderName.slice(1);
  ...
  function openAddBox() {
    setIsAddBoxActive(!isAddBoxActive);
    refInput.current.focus();
  }

  return (
    <div className="sidebar__add-box">
      <button className="sidebar__btn-add" onClick={openAddBox}>
        <svg
          className="sidebar__btn-add-icon"
          width="12"
          height="12"
          viewBox="0 0 12 12"
          fill="none"
        ...
        <span className="sidebar__btn-text">Добавить папку</span>
      </button>

      <div
        className={`sidebar__add-form-box ${isAddBoxActive ? "active" : ""}`}
      >
        <input
          ...
          ref={refInput}
        />
        <ul className="sidebar__add-color-list">
          {db.colors.map(color => (
            <li
              key={color.hex}
              className={`sidebar__add-color-item ${
                color.hex === addColor ? "active" : ""
              }`}
        ...
        <button className="sidebar__add-btn" onClick={addFolder}>
          Добавить
        </button>
        <button
          className="sidebar__add-close"
          onClick={() => setIsAddBoxActive(false)}
        >
          <svg
            width="21"
            height="21"
