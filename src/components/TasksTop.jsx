...
function TasksTop({ data, setData, listData }) {
  ...
  function changeTitle() {
    ...
    titleRef.current.value = listData.name;
    titleRef.current.focus();
  }

  function acceptChangeTitle() {
    const featuredData = data.map(item => {
      if (item.id === listData.id) {
        item.name = titleRef.current.value;
      }
  ...
  return (
    <div className="tasks__top">
      <h3
        className={`tasks__title ${isTitleChanging ? "hidden" : ""}`}
        style={{ color: `${listData.color}` }}
      >
        {listData.name}
      </h3>
      <div
        className={`tasks__title-change-box ${isTitleChanging ? "active" : ""}`}
      >
        <input
          ref={titleRef}
          className="tasks__title-change-input"
          style={{ color: `${listData.color}` }}
          type="text"
        />
        <div className="tasks__title-change-svg-box">
          <svg
            className="tasks__title-change-accept"
            onClick={acceptChangeTitle}
          ...
          <svg
            className="tasks__title-change-close"
            onClick={changeTitle}
            width="14"
            height="14"
            viewBox="0 0 11 11"
      ...
      <svg
        className={`tasks__title-change ${isTitleChanging ? "hidden" : ""}`}
        onClick={changeTitle}
        width="15"
        height="15"
        viewBox="0 0 15 15"
